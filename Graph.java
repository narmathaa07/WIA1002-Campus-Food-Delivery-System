...
public class Graph {
    ...
    // NEW: Delete a location and all connected edges
    public boolean deleteNode(String location) {
        if (!adjacencyList.containsKey(location)) {
            return false; // Location doesn't exist
        }
        
        // 1. Remove all edges TO this location from other nodes
        for (int i = 0; i < nodes.size(); i++) {
            String node = nodes.get(i);
            if (node != null && !node.equals(location)) {
                CustomArrayList<Edge> edges = adjacencyList.get(node);
                if (edges != null) {
                    // Remove edges pointing to the location being deleted
                    for (int j = edges.size() - 1; j >= 0; j--) {
                        Edge edge = edges.get(j);
                        if (edge != null && edge.destination.equals(location)) {
                            edges.remove(j);
                        }
                    }
                }
        ...
        // 2. Remove the node from adjacency list
        adjacencyList.remove(location);
        
        // 3. Remove from nodes list
        for (int i = 0; i < nodes.size(); i++) {
            if (nodes.get(i) != null && nodes.get(i).equals(location)) {
                nodes.remove(i);
                break;
            }
        }
    ...
    // NEW: Delete a specific route between two locations
    public boolean deleteEdge(String source, String destination) {
        ...
        CustomArrayList<Edge> sourceEdges = adjacencyList.get(source);
        if (sourceEdges != null) {
            for (int i = sourceEdges.size() - 1; i >= 0; i--) {
                Edge edge = sourceEdges.get(i);
                if (edge != null && edge.destination.equals(destination)) {
        ...
        // Remove reverse edge (undirected graph)
        CustomArrayList<Edge> destEdges = adjacencyList.get(destination);
        if (destEdges != null) {
            for (int i = destEdges.size() - 1; i >= 0; i--) {
                Edge edge = destEdges.get(i);
                if (edge != null && edge.destination.equals(source)) {
                    destEdges.remove(i);
                    break;
                }
            }
    ...
    // Get all nodes for combo box
    public CustomArrayList<String> getAllNodes() {
        return nodes;
    }
    ...
    // Get edges for a location
    public CustomArrayList<Edge> getEdges(String location) {
